...
public protocol GWLMPinViewTextDelegate: AnyObject {
...
@available(iOS 9.0, *)
@IBDesignable
public class GWLMPinView: UIView {

    private var stackView = UIStackView()
    private var textFeilds = [GWLMPinTextField]()
    public weak var fieldDelegate: GWLMPinViewTextDelegate?
    ...
    */
    ...
    func initilize() {
        ...
        _ = bounds.width - (CGFloat(passcodeLength)*spacing)
//        assert(requiredFieldBoxSize > spacing, "Pin text box area should be greater than 'fieldSpacing' property value")
        ...
        var fields = [GWLMPinTextField]()
        var i = 100
        for f in 1...passcodeLength {
            let field = GWLMPinTextField()
            field.tag = i
            i = i+1
            if f == 1 {
        ...
        for (_, item) in fields.enumerated() {
